          ...
          <div className="p-5 bg-white min-h-screen animate-in fade-in relative z-20">
            ...
            {adminTab === 'dashboard' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                {/* 14. สรุปรายรับรายวัน */}
                <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs">
                   <h3 className="font-bold text-xs text-slate-800 mb-3 border-b border-slate-100 pb-2.5 flex items-center gap-2"><Clock size={16}/> สรุปรายรับรายวัน (7 วันล่าสุด)</h3>
                   <div className="space-y-2.5">
                      {revData.dailyHistory.map((d, idx) => (
                         <div key={idx} className="flex justify-between items-center text-xs">
                            <span className={idx === 0 ? "font-bold text-amber-800" : "text-slate-500 font-bold"}>{idx === 0 ? `วันนี้ (${d.date})` : d.date}</span>
                            <span className={`font-black ${idx === 0 ? "text-amber-800" : "text-slate-900"}`}>฿{d.total.toLocaleString()}</span>
                         </div>
                      ))}
                   </div>
                </div>
