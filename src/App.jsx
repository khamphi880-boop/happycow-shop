...
export default function App() {
  ...
  // --- เพิ่มฟังก์ชันจัดการอัปเดตสถานะจัดส่ง (แก้ไขจุดที่พัง) ---
  const handleConfirmDelivery = async () => {
    ...
    try {
      await updateDoc(doc(db, 'orders', deliveryModal.id), {
        ...
        deliveryMessage: deliveryLocation === 'room' ? 'จัดส่งที่หน้าห้องเรียบร้อยแล้วค่ะ 🛵' : (deliveryLocation === 'building' ? 'จัดส่งที่หน้าตึกเรียบร้อยแล้วค่ะ 🛵' : 'ลูกค้ารับสินค้าที่ร้านเรียบร้อยค่ะ 🐮')
      });

      const successText = `✅ ส่งน้ำเรียบร้อยแล้วจ้า!\nบิล: #${deliveryModal.id.slice(0,6)}\nลูกค้า: คุณ ${deliveryModal.lineName}\nจุดส่ง: ${deliveryLocation === 'room' ? 'หน้าห้อง' : (deliveryLocation === 'building' ? 'หน้าตึก' : 'รับเองที่ร้าน')}\n\nขอบคุณที่อุดหนุนวัวนมอารมณ์ดีนะคะ 🐮💖`;
      ...
      if (window.liff && window.liff.isApiAvailable('shareTargetPicker')) {
          ...
          if(res) showAlert('บันทึกการส่งและแชร์สถานะให้ลูกค้าเรียบร้อย! 🎉');
          else showAlert('อัปเดตสถานะแล้ว (คุณยกเลิกการแชร์ข้อความ)');
      } else {
          setAdminDeliverySuccessData({ text: successText });
      }
  ...
  // --- เพิ่มฟังก์ชันคำนวณรายรับ (แก้ไขจุดที่พัง) ---
  const calculateRevenue = () => {
    const now = new Date();
    let daily = 0, monthly = 0, yearly = 0;
    
    // สร้าง Array เก็บข้อมูลย้อนหลัง 7 วัน
    ...
    if (orders && orders.length > 0) {
      orders.filter(o => o.status === 'completed').forEach(o => {
        ...
        // คำนวณรายวัน
        if (d.toDateString() === now.toDateString()) daily += o.total;

        // หยอดรายได้ลงกราฟประวัติ 7 วัน
        const histIndex = dailyHistory.findIndex(h => h.rawDate === d.toDateString());
        if (histIndex !== -1) dailyHistory[histIndex].total += o.total;
      });
    }
  ...
  // --- เพิ่มฟังก์ชันดาวน์โหลดรายรับ CSV (แก้ไขจุดที่พัง) ---
  const exportToCSV = () => {
    if (!orders || orders.length === 0) return showAlert('ไม่มีข้อมูลรายรับให้ Export ครับ');
    const headers = ['Order ID', 'วันที่', 'ชื่อลูกค้า', 'รายการ', 'ยอดรวม (บาท)', 'ช่องทางชำระ', 'สถานะ'];
    const rows = orders.map(o => [
      o.id,
      new Date(o.timestamp).toLocaleString('th-TH'),
      `"${o.lineName || 'ลูกค้าทั่วไป'}"`,
      `"${(o.items||[]).map(i => `${i.qty}x ${i.name}`).join(', ')}"`,
      o.total,
      o.paymentMethod === 'cash' ? 'เงินสด' : (o.paymentMethod === 'promptpay' ? 'พร้อมเพย์' : 'ไทยช่วยไทยพลัส'),
      o.status
    ]);
    ...
    link.setAttribute("download", `revenue_report_${new Date().toLocaleDateString('en-CA')}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // --- เพิ่มฟังก์ชันดาวน์โหลดเมนู CSV (แก้ไขจุดที่พัง) ---
  const exportMenuToCSV = () => {
    if (!menuItems || menuItems.length === 0) return showAlert('ไม่มีข้อมูลเมนูให้ Export ครับ');
    const headers = ['ชื่อเมนู', 'หมวดหมู่', 'ราคา (บาท)', 'บวกเพิ่มปั่น (บาท)', 'สถานะ', 'เป็นเมนูแนะนำ'];
    const rows = menuItems.map(m => [
      `"${m.name}"`,
      `"${m.category}"`,
      m.price,
      m.blendPrice || 0,
      m.isSoldOut ? 'หมดชั่วคราว' : 'พร้อมขาย',
      m.isPromoted ? 'ใช่' : 'ไม่ใช่'
    ]);
    const csvContent = "data:text/csv;charset=utf-8,\uFEFF" + [headers, ...rows].map(e => e.join(",")).join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `menu_list_${new Date().toLocaleDateString('en-CA')}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
